...
class SpatioTemporalKMeans(kmeans):
    def __init__(self, config, graph, graph_str, datas, initial_centers=None, k=8,
        ...
        if not initial_centers:
            ...
            # ica
            initial_centers = random_center_initializer(datas, k).initialize() 
        
        print("initialize finished-------------")
        metric = distance_metric(type_metric.USER_DEFINED, func=self.distance)

        super().__init__(datas, initial_centers, config, config["tolerance"], ccore, metric=metric)
        self.cr = config["cr"]
        self.cd = config["cd"]
        self.cp = config["cp"]
        self.cs = config["cs"]
        ...
        # acc
        # self.ranges = [2.45122418, 1.85,       0.82592873, 2.16      ]
        # lka
        # self.ranges = [2.7977348,  3.96,       0.81142169, 1.81      ]
        # ica
        self.ranges = [2.7977348,  3.96,       0.81363001, 1.81      ]
...
if __name__ == '__main__':
    ...
    # # # 将字典保存到文件
    # # with open('kmeans_model/acc_td3_risk/spatio_temporal_graph.json', 'w') as file:
    # #     json.dump(graph_str_keys, file, indent=2)

    # # 
    # # # 将字典保存到文件
    # # with open('kmeans_model/acc_td3_risk/spatio_temporal_attr_dic.json', 'w') as file:
    # #     json.dump(attr_dic_str_keys, file, indent = 2)

    # # 保存字典到文件
    # # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/acc_td3_risk/silhouette_graph.json', 'w') as file:
    # #     json.dump(graph_str_keys, file, indent=2)

    # # # 保存字典到文件
    # # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/acc_td3_risk/silhouette_attr_dic.json', 'w') as file:
    # #     json.dump(attr_dic_str_keys, file, indent = 2)
    ...
    # kmeans_instance = SpatioTemporalKMeans(config=eval_config, datas=states, graph=attr_dic, graph_str=attr_dic_str ,k=1260)
    # #   进行聚类
    # kmeans_instance.process()
    # # 
    # #   模型保存
    # # joblib.dump(kmeans_instance, 'kmeans_model/acc_td3_risk/spatio_temporal_Kmeans.pkl')
    # joblib.dump(kmeans_instance, './data_analysis/mdp/second_stage_abstract/kmeans_model/acc_td3_risk/spatio_temporal_gap_Kmeans.pkl')
    ...
    # #   获得图和状态
    # graph, attr_dic, attr_dic_str = get_mdp_map("./data_analysis/mdp/first_stage_abstract/lka_datasets/first_abstract_pro_center_data.csv", eval_config)
    ...
    # # 保存字典到文件
    # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/lka_td3_risk/ica_graph.json', 'w') as file:
    #     json.dump(graph_str_keys, file, indent=2)

    # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/lka_td3_risk/ica_attr_dic.json', 'w') as file:
    #     json.dump(attr_dic_str_keys, file, indent = 2)
    
    
    # # optimal_k = find_optimal_k_elbow(states, min_clusters=1200, max_clusters=1500, 
    # #                                  save_path='./data_analysis/mdp/second_stage_abstract/kmeans_model/lka_td3_risk/lka_td3_clusters/elbow')

    # kmeans_instance = SpatioTemporalKMeans(config=eval_config, datas=states, graph=attr_dic, graph_str=attr_dic_str ,k=4)
    
    # #   进行聚类
    # kmeans_instance.process()

    # #   模型保存
    # joblib.dump(kmeans_instance, './data_analysis/mdp/second_stage_abstract/kmeans_model/lka_td3_risk/spatio_temporal_canopy_Kmeans.pkl')
    ...
    #   获得图和状态
    graph, attr_dic, attr_dic_str = get_mdp_map("./data_analysis/mdp/first_stage_abstract/ica_datasets/first_abstract_pro_center_data.csv", eval_config)
    ...
    # 保存字典到文件
    # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/ica_dqn_risk/ica_graph.json', 'w') as file:
    #     json.dump(graph_str_keys, file, indent=2)

    # with open('./data_analysis/mdp/second_stage_abstract/kmeans_model/ica_dqn_risk/ica_attr_dic.json', 'w') as file:
    #     json.dump(attr_dic_str_keys, file, indent = 2)
    ...
    kmeans_instance = SpatioTemporalKMeans(config=eval_config, datas=states, graph=attr_dic, graph_str=attr_dic_str ,k=3)
    
    #   进行聚类
    kmeans_instance.process()

    #   模型保存
    joblib.dump(kmeans_instance, './data_analysis/mdp/second_stage_abstract/kmeans_model/ica_dqn_risk/spatio_temporal_gap_Kmeans.pkl')