...
class IntersectionEnv(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for t in range(n_vehicles - 1):
            self._spawn_vehicle(np.linspace(0, 80, n_vehicles)[t], spawn_probability=1, position_deviation=0, speed_deviation=0)
        # for _ in range(simulation_steps):
        #     [(self.road.act(), self.road.step(1 / self.config["simulation_frequency"])) for _ in range(self.config["simulation_frequency"])]

        # Challenger vehicle
        self._spawn_vehicle(60, spawn_probability=1, go_straight=True, position_deviation=0.5, speed_deviation=0.5)
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(60 + 0*self.np_random.standard_normal(), 0),  # numpy version problem (randn(1))
                             speed=7,
    ...
    def _spawn_vehicle(self,
        ...
        if self.np_random.random() > spawn_probability:  # numpy version problem (rand())
            return
        ...
        vehicle = vehicle_type.make_on_lane(self.road, ("o" + str(route[0]), "ir" + str(route[0]), 0),
                                            longitudinal=longitudinal + 5 + self.np_random.standard_normal() * position_deviation,  # numpy version problem (randn())
                                            speed=8 + self.np_random.standard_normal() * speed_deviation)  # numpy version problem (randn())
