...
def parse_args():
    ...
    parser.add_argument("--model", type=str, help="the path of model to use", 
                        default="data_analysis/mdp/second_stage_abstract/kmeans_model/acc_td3_risk/spatio_temporal_elbow_Kmeans.pkl")
...
def get_cost(info, state, action, done, truncated, last_action=None, last_state=None):
    ...
    rel_distance = state[0][1] - state[1][1]
    
    if info["crashed"] or rel_distance < safe_distance:
        cost = 100
...
def choose_action_with_model(state, agent, eval_config, mdl, centers, agent_weight, model_weight):
    ...
    return agent_weight * agent_choice + model_weight * model_choice  # TODO: Choose best weights
...
def train(env, eval_env, agent, max_timesteps, max_updates, print_freq, save_freq,
    ...
    # 每个episode的小于安全距离的次数
    episode_is_too_close_list = list()
    ...
    for t_step in range(max_timesteps):
        ...
        """Select action according to policy and model"""
        beg_time = time.time()
        ...
        # print("cost:", cost)
        is_too_close = 1 if (next_state[0][1] - next_state[1][1]) < safe_distance else 0
        ...
        if "risk" in algo_config["algo_name"]:
            ...
            logging.info('%s, %s, %s, %s, %s, %s, %s',
                         episode_num, state, action, reward, next_state, done, cost)
        else:
            agent.memory.push(state, action, reward, next_state, done)
            logging.info('%s, %s, %s, %s, %s, %s',
                         episode_num, state, action, reward, next_state, done)

        # 危险场景库
        if cost == 100 and "risk" in algo_config["algo_name"]:
            flag = False
        ...
        episode_is_too_close += is_too_close
        # env.render()
        ...
        if done or truncated:
            ...
            episode_is_too_close_list.append(episode_is_too_close)
            ...
            writer.add_scalar('relative_distance/too_close_count', episode_is_too_close, global_step=episode_num)
            
            steps.append(episode_num)
            values.append(episode_reward)
            avg_values.append(np.mean(episode_reward))
            crashes.append(sum(episode_crash_list))
            distances.append(episode_is_too_close)
            ...
            logging.info('发生撞车：Step:%s, Episode: %s, Episode Step：%s, Reward：%s, crash Num:%s',
                         t_step, episode_num, episode_timesteps, episode_reward, sum(episode_crash_list))
        ...
        if t_step % algo_config["trainer"]["eval_freq"] == 0:
            with torch.no_grad():
                ...
                ave_reward, num_crash = eval_policy(eval_config, mdl, centers, agent_weight, model_weight, agent, 
                ...
                logging.info('Step:%s, Episode Num:%s, Ave Reward:%s, crash Num:%s',
                             t_step, eval_episodes, ave_reward, num_crash)
    ...
    # 保存数据
    data_save_dir = 'acc/data/risk/td3/with_model_data/data'
    plot_save_dir = 'acc/data/risk/td3/with_model_data/plot'
    os.makedirs(data_save_dir, exist_ok=True)
    os.makedirs(plot_save_dir, exist_ok=True)
    
    format_time = time.strftime("%Y%m%d%H%M", time.localtime())
    utils.save_csv_on_models({ 'Step': steps, 'Value': values, 'Avg_Value': avg_values, 'Crash': crashes, 'Distance': distances }, data_save_dir + '/{}.csv'.format(format_time))
    utils.single_plot(steps, values, plot_save_dir + '/{}.png'.format(format_time), xlabel="episodes", ylabel="return")
    utils.single_plot(steps, crashes, plot_save_dir + '/crash_{}.png'.format(format_time), xlabel="episodes", ylabel="crash")
    utils.single_plot(steps, distances, plot_save_dir + '/distance_{}.png'.format(format_time), xlabel="episode", ylabel="distances")
    ...
    return episode_reward_list, episode_crash_list, episode_step_list, episode_is_too_close_list
...
def main():
    ...
    episode_reward_list, episode_crash_list, episode_step_list, episode_is_too_close_list = train(env, eval_env, agent,
    ...
    np.save(os.path.join(config.project, "results", "train_epoch_is_too_close.npy", episode_is_too_close_list))
    ...
    utils.single_plot(episode_step_list, episode_is_too_close_list,
                      path=os.path.join(config.project, "plots", "train_step_is_too_close"),
                      xlabel="Step", ylabel="Num Of too close distance", title="", label=algo_config["algo_name"])
    utils.single_plot(None, episode_is_too_close_list,
                      path=os.path.join(config.project, "plots", "train_epoch_is_too_close"),
                      xlabel="Epoch", ylabel="Num Of too close distance", title="", label=algo_config["algo_name"])
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        ...
        format='%(message)s',
        handlers=[
            logging.FileHandler('acc/data/trained_acc_data_with_model.log'),  # 输出到文件
