...
def parse_args():
    ...
    parser.add_argument("--project", type=str, help="the path to save project",
                        default="project/20240102-DDPG_risk-acc-with-model")
    parser.add_argument("--model_config", type=str, help="path of rl algorithm configuration",

                        default="conf/algorithm/DDPG_risk_acc.yaml")
...
def build_agent(config, env, gpu, writer):
    ...
    if config["algo_name"] == "TD3":
        ...
        agent = DDPG(config, env.action_space.high[0], gpu, writer=writer)
    elif config["algo_name"] == "DDPG_risk":
        agent = DDPG_risk(config, env.action_space.high[0], gpu, writer=writer)
...
def train(env, eval_env, agent, max_timesteps, max_updates, print_freq, save_freq, 
    ...
    # 保存数据
    data_save_dir = 'acc/data/risk/ddpg/risk_with_model_data/data'
    plot_save_dir = 'acc/data/risk/ddpg/risk_with_model_data/plot'
