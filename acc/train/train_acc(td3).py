...
def parse_args():
    ...
    parser.add_argument("--project", type=str, help="the path to save project",
                        default="project/20231026-TD3-acc")
...
def get_cost(info, state, action, done, truncated, last_action=None, last_state=None):
    ...
    rel_distance = state[0][1] - state[1][1]  # relative distance
...
def train(env, eval_env, agent, max_timesteps, max_updates, print_freq, save_freq, env_config, algo_config, project_dir,
    ...
    for t_step in range(max_timesteps):
        ...
        if done or truncated:
            ...
            logging.info('发生撞车：Step:%s, Episode: %s, Episode Step：%s, Reward：%s, crash Num:%s, too close Num: %s', 
                         t_step, episode_num, episode_timesteps, episode_reward, sum(episode_crash_list), episode_is_too_close)
    ...
    # 保存数据
    data_save_dir = 'acc/data/risk/td3/data/data'
    plot_save_dir = 'acc/data/risk/td3/data/plot'
    os.makedirs(data_save_dir, exist_ok=True)
    os.makedirs(plot_save_dir, exist_ok=True)
    
    format_time = time.strftime("%Y%m%d%H%M", time.localtime())
    utils.save_csv_on_models({ 'Step': steps, 'Value': values, 'Crash': crashes, 'Distance': distances }, data_save_dir + '/{}.csv'.format(format_time))
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        ...
        format='%(message)s',
        handlers=[
            logging.FileHandler('acc/data/trained_acc_data.log'),  # 输出到文件
