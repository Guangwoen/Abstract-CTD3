...
def parse_args():
    ...
    parser.add_argument("--project", type=str, help="the path to save project", 
                        ...
                        default="project/20240105-TD3_risk-lanekeeping-with-model")
...
def get_cost(info, state, action, done, truncated, last_action=None, last_state=None):
    ...
    if done and not info["crashed"] and not truncated:
        cost = 100
    # if abs(state[0][0]) > safe_distance:
    #     cost = 100
    return cost
...
def train(env, eval_env, agent, max_timesteps, max_updates, print_freq, save_freq, 
    ...
    # 保存数据
    data_save_dir = 'lanekeeping/data/risk/td3/risk_with_model_data/data'
    plot_save_dir = 'lanekeeping/data/risk/td3/risk_with_model_data/plot'
    os.makedirs(data_save_dir, exist_ok=True)
    os.makedirs(plot_save_dir, exist_ok=True)
    
    format_time = time.strftime("%Y%m%d%H%M", time.localtime())
    utils.save_csv_on_models({ 'Step': steps, 'Value': values, 'Avg_Value':avg_values, 'Outoflane': outoflanes, 'Distance': distances}, 
                             data_save_dir + '/{}.csv'.format(format_time))
    ...
    utils.single_plot(steps, distances, plot_save_dir + '/distance_{}.png'.format(format_time), xlabel="episodes", ylabel="distance")
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        ...
        format='%(message)s',
        handlers=[
            logging.FileHandler('lanekeeping/data/trained_lka_data_with_model.log'),  # 输出到文件
